...
import React, { useState } from 'react';
import useLogin from '../../hooks/useLogin';
import { sendPasswordMail } from '../../services/yourApiFunctions';
...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onSwitchToRegister }) => {
    ...
    const validateEmail = (email: string): string => {
        ...
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) return 'Email không đúng định dạng (ví dụ: user@example.com)';
        return '';
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isForgotPassword) {
            try {
                const { success, result } = await sendPasswordMail(email);
                if (success && result === 0) {
                    ...
                    setLocalError('❌ Gửi email thất bại. Vui lòng thử lại sau.');
                }
            } catch (error) {
                console.error(error);
                setLocalError('❌ Lỗi hệ thống. Vui lòng thử lại sau.');
            }
            return;
        }
        ...
        try {
            ...
            if (authResponse && authResponse.token) {
                ...
                setLocalError('❌ Đăng nhập thất bại. Vui lòng kiểm tra lại.');
            }
        } catch (err: any) {
    ...
    return (
        <div id="loginModal" className="fixed inset-0 z-50 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && handleClose()}>
            <div className="flex items-center justify-center min-h-screen text-center">
                <div className="fixed inset-0 bg-gray-500 opacity-75" aria-hidden="true"></div>

                <div className="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-md sm:w-full z-50 p-6 relative">
                    {/* Nút đóng */}
                    <button
                        onClick={handleClose}
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        {/* Thông báo lỗi */}
                        {localError && (
                            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md text-sm text-red-700 font-medium text-center">
                                {localError}
                            </div>
                        )}
                        ...
                        <button
                            type="submit"
                            className={`w-full py-2 px-4 rounded-md text-white font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors ${
                                successMessage
                                    ? 'bg-green-600 hover:bg-green-700'
                                    : 'bg-indigo-600 hover:bg-indigo-700'
                            }`}
                            disabled={loading || !!successMessage}
                        >
                            {loading ? 'Đang xử lý...' : isForgotPassword ? 'Gửi yêu cầu' : 'Đăng nhập'}
                        </button>
